...
An organization can enter its own R2 bucket under **Organization → Integrations → S3**. Use that bucket's R2 S3 endpoint, region `auto`, bucket name, and bucket-scoped object read and write credentials. Set CORS on the organization bucket separately; the `o37-cap` policy does not apply to it. The **Test** button checks `HeadBucket` from `cap-web`; it does not test browser CORS, uploads, playback, or editing. On 2026-09-24, the owner's test returned a production React error while Railway logged `Failed to connect to S3`. A read-only `HeadBucket` check from the running web service succeeded for the default `o37-cap` bucket, so the specific organization bucket failure is still unverified. The fork now returns expected connection failures to the form, logs the S3 error name and HTTP status without credentials, and uses path-style addressing for the R2 test. Retest the organization bucket after deployment, then upload and play a small recording before switching real work to it.
